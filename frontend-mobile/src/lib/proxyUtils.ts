...
export function containerStatusColor(status: string | null | undefined): 'default' | 'success' | 'warning' | 'danger' | 'accent' {
  if (!status) return 'default';
  const s = status.toLowerCase();
  if (s === 'running' || s === 'r' || s === 'healthy' || s === 'h' || s.includes('good')) return 'success';
  if (s === 'stopped' || s === 's') return 'default';
  if (s.includes('error') || s === 'e' || s === 'u') return 'danger';
  return 'accent';
}
...
export function proxyStatusColor(status: string): 'default' | 'success' | 'warning' | 'danger' | 'accent' {
  switch (status) {
